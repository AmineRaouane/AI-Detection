...
export function Detector() {
        ...
        try {
            ...
            switch (activeTab) {
                case 'text':
                    ...
                    requestOptions.body = JSON.stringify({ file: content.trim() });  // Changed this line
                    break;
    ...
    const isAnalyzeEnabled = !loading && (content.trim() || image || audio || video);
    ...
    const getResultDisplay = () => {
        ...
        if (result) {
            ...
            return (
                <div className={`w-full p-4 rounded-lg text-center font-semibold ${isAIGenerated ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'
                    }`}>
        ...
        <div className="min-h-screen bg-inherit text-white p-6">
            <div className="max-w-4xl mx-auto">
                {/* <header className="text-center mb-12">
                    <h1 className="text-5xl font-bold bg-clip-text mb-4 text-transparent bg-gradient-to-r from-purple-400 to-pink-600">
                        AI Content Detector
                    </h1>
                ...
                <Card className="bg-inherit border-purple-500/20 backdrop-blur-lg hover:border-purple-500/40 transition-all rounded-lg p-6">
                    <Tabs defaultValue="text" className="w-full">
                        <TabsList className="grid grid-cols-4 gap-4 bg-gray-900/50 p-1">
                            <TabsTrigger value="text" className="data-[state=active]:bg-purple-500 text-white" onClick={() => handleTabSwitch('text')}>
                                Text
                            </TabsTrigger>
                            <TabsTrigger value="image" className="data-[state=active]:bg-purple-500 text-white" onClick={() => handleTabSwitch('image')}>
                                Image
                            </TabsTrigger>
                            <TabsTrigger value="audio" className="data-[state=active]:bg-purple-500 text-white" onClick={() => handleTabSwitch('audio')}>
                                Audio
                            </TabsTrigger>
                            <TabsTrigger value="video" className="data-[state=active]:bg-purple-500 text-white" onClick={() => handleTabSwitch('video')}>
                                Video
                            </TabsTrigger>
                        </TabsList>
